...
class SerialApp:
    def __init__(self, master):
        ...
        sport_menu = ttk.OptionMenu(self.master, self.port_var, *ports)
        self.port_menu.place(x=10, y=30)
        helv36 = tkFont.Font(family='Helvetica', size=30, weight=tkFont.BOLD)
        # self.port_menu.config(style=helv36)
        helv36 = tkFont.Font(family='Helvetica', size=36)

        options = 'eggs spam toast'.split()
        selected = tk.StringVar(self.master, value=options[0])
        choose_test = tk.OptionMenu(self.master, selected, *options)
