    ...
    def received_data(self):
        ...
        try:
            if self.ser.isOpen():
                ...
                time.sleep(0.5) 
                for i in range(0,2):  // 2 is the number of times the data is received
                    if VOLTAGE_FLAG:
                        self.send_data(self.ser, "VOUT1?")
                        decimal_value = self.receive_voltage()
                        print("voltage:", decimal_value)
                        time.sleep(0.5)
                        VOLTAGE_FLAG = False
                    else:
                        self.send_data(self.ser, "IOUT1?")
                        decimal_value = self.receive_current()
