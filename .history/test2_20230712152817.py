...
class SerialApp:
    def __init__(self, master):
        ...
        self.save_interval = self.interval_var.get()
        ...
        self.notelabel["text"] = "Warning: Make sure the port is connected !"
    ...
    def start(self):
        ...
        while self.port == "COM5" or self.port == "COM6":
            self.master.after(0, self.update_notelabel, "Warning: Make sure the port is correct !")
            return
        # self.master.after(0, self.update_notelabel, "Connected with " + self.port + " with save format " + self.format_var.get() + " and preiod " + self.interval_var.get() + "s")
        self.notelabel["text"] = "Connected with " + self.port + " with save format " + self.save_format + " and preiod " + self.save_interval + "s"
        # 打开串口
        self.running = True
    ...
    def clear(self):
        ...
        self.port = self.port_var.get()
        self.save_interval = int(self.interval_var.get())
        self.save_format = self.format_var.get()
        self.running = True
        self.paused = False
