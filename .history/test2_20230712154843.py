...
class SerialApp:
    def __init__(self, master):
        ...
        formatter = FuncFormatter(self.three_decimal)
        ...
        # start button
        self.GButton_234=tk.Button(master)
        self.GButton_234["bg"] = "#f0f0f0"
        ft = tkFont.Font(family='Times',size=10)
        self.GButton_234["font"] = ft
        self.GButton_234["fg"] = "#000000"
        self.GButton_234["justify"] = "center"
        self.GButton_234["text"] = "Start"
        self.GButton_234.place(x=30,y=110,width=105,height=55)
        self.GButton_234["command"] = self.start
        ...
        GButton_430["command"] = self.Continue
        ...
        self.notelabel["text"] = "Note: Make sure the port is connected !"
    ...
    def start(self):
        ...
        self.save_format = self.format_var.get()
        self.interval = int(self.interval_var.get())
        if self.startthread != None and  self.startthread.is_alive():
            self.master.after(0, self.clear)
            self.clear()
            return 
        self.port = self.port_var.get()
        while self.port == "COM5" or self.port == "COM6":
            self.master.after(0, self.update_notelabel, "Error: Make sure the port is correct !")
            return
        # open the serial port
        self.running = True
        self.startthread = threading.Thread(target=self.receive_data,daemon=True)
        self.startthread.start()

    def update_notelabel(self, text):
    ...
    def clear(self):
        ...
        # clear the plot
        #clear the ax1 ax2 ax3 number but not clear the label
        self.ax1.clear()
        self.ax2.clear()
        self.ax3.clear()
        # set the ax1 ax2 ax3 label  
        # reset labels
    self.ax1.set_ylabel('Voltage(V)')
    self.ax2.set_ylabel('Current(A)')
    self.ax3.set_xlabel('Time(s)')
           ...
           while self.running and not self.pause:
            try:
                ...
                if voltage is None or current is None:
                    ...
                    continue
                if self.port == "COM5" or self.port == "COM6":
                    self.master.after(0, self.update_notelabel, "Error: Make sure the port is correct !")
                    self.clear()
                    self.running = False
                    break
                else:
                    self.master.after(0, self.update_notelabel, "Connected with " + self.port + " with save format " + self.save_format + " and preiod " + str(self.interval) + "s")
                ...
                self.text.delete('1.0', tk.END)
                self.text.insert(tk.END, "\n".join([f"{t:.2f}s: {d1:.2f}V, {d2:.3f}A, {d3:.3f}W" for t, d1, d2, d3 in zip(self.times[-60:], self.voltage[-60:], self.current[-60:], self.power[-60:])]))
