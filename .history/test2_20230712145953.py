...
class SerialApp:
    ...
    def start(self):
        ...
        self.master.after(0, self.update_notelabel, "Connected with " + self.port + " with save format " + self.format_var.get() + " and preiod " + self.interval_var.get() + "s")
         "Connected with " + self.port + " with save format " + self.format_var.get() + " and preiod " + self.interval_var.get() + "s"
        self.interval = int(self.interval_var.get())
        # 打开串口
        self.running = True
        self.startthread = threading.Thread(target=self.receive_data)
