...
class SerialApp:
    def __init__(self, master):
        ...
        self.start_button = ttk.Button(self.master, text="S", command=self.start)
        ...
        self.text = tk.Text(self.master, height=15, width=50)
        self.text.grid(row=0, column=3, rowspan=5)
    ...
    def receive_data(self):
        ...
        while self.running:
            ...
            self.text.insert(tk.END, "\n".join([f"{t:.2f}s: {d1:.2f}V, {d2:.2f}A, {d3:.2f}mW" for t, d1, d2, d3 in zip(self.times[-15:], self.data1[-15:], self.data2[-15:], self.data3[-15:])]))
